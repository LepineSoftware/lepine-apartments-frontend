...
export default async function handler(req, res) {
  ...
  const comingSoonQuery = qs.stringify({
    publicationState: "live",
    populate: ["hero.content.image", "address.city", "address.coords"],
    encodeValuesOnly: true,
  });
  ...
  propertiesData.data.forEach((property) => {
    ...
    const order = propertyOrder.filter((p) => p.pageId === pageId)[0]?.order
      ? propertyOrder.filter((p) => p.pageId === pageId)[0].order
      : null;

    const p = {
      name,
      address,
      city: address?.city?.data?.attributes?.name
        ? address?.city?.data?.attributes?.name
        : null,
      theme: pageId,
      href: `/property/${pageId}`,
      image: hero?.content?.image?.data?.attributes?.url
        ? baseUrl(hero.content.image.data.attributes.url)
        : null,
      svg: logos?.light?.data?.attributes?.url
        ? baseUrl(logos.light.data.attributes.url)
        : null,
      order,
    };
